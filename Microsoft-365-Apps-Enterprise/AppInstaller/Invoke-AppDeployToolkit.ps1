...
.SYNOPSIS
PSAppDeployToolkit v4 wrapper for Microsoft 365 Apps for Enterprise.

.DESCRIPTION
Built from Install-Tools\Invoke-AppDeployToolkit_M365-Template.ps1.
...
DIFFERENCES FROM THE STOCK Frontend\v4 TEMPLATE:
- The module import path is "$PSScriptRoot\PSAppDeployToolkit\..." instead of
  "$PSScriptRoot\..\..\..\PSAppDeployToolkit\...". The stock path only resolves while the
  file sits inside the extracted toolkit; in a package root it silently falls through to
  PSModulePath and fails with exit code 60008 on a device that has no toolkit installed.
- A -ProductID parameter is added and excluded from Open-ADTSession, which does not accept
...
# fails the build if this file is not present in Files\ after a sync.
